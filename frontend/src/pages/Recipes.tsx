...
export default function RecipesPage() {
  ...
  const [savedFilter, setSavedFilter] =
    useState<(typeof savedFilters)[number]>("all");
  ...
  const shortcuts = {
    ArrowRight: () => {
      ...
      setFocusedIndex((prev) =>
        Math.min(prev + 1, displayedRecipes.length - 1),
      );
    },
    ArrowLeft: () => {
      if (!displayedRecipes?.length) return;
    ...
    ArrowDown: () => {
      if (!displayedRecipes?.length) return;
      setFocusedIndex((prev) =>
        Math.min(prev + currentCols, displayedRecipes.length - 1),
      );
    },
    ArrowUp: () => {
      if (!displayedRecipes?.length) return;
    ...
    <main>
      <header className="px-4 pt-4">
        ...
        <p className="text-muted-foreground mt-1">
          Search and filter to find the perfect meal.
        </p>
      </header>
      ...
      <section className="px-4 mt-6">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          ...
          <Select value={savedFilter} onValueChange={(v) => setSavedFilter(v)}>
            <SelectTrigger aria-label="Filter by saved status">
              <SelectValue placeholder="Saved" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">Saved and unsaved</SelectItem>
          ...
          <Select value={diet} onValueChange={(v) => setDiet(v)}>
            <SelectTrigger aria-label="Filter by diet">
          ...
          <Select value={maxTime} onValueChange={setMaxTime}>
            <SelectTrigger aria-label="Max cooking time">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {isLoading ? (
            ...
            displayedRecipes.map((recipe, index) => (
              <div key={recipe.id} ref={(el) => (cardRefs.current[index] = el)}>
                <RecipeCard
                  recipe={recipe}
