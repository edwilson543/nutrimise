...
import {
  ...
  listSavedRecipesRecipesSavedGet,
} from "@/lib/client";
...
export const useRecipeList = () => {
  const recipeList = useQuery({
    queryFn: () =>
      listRecipesRecipesGet().then((response) => response.data.recipes),
    queryKey: [queryKeys.RECIPE_LIST],
  });

  const savedRecipes = useQuery({
    queryFn: () =>
      listSavedRecipesRecipesSavedGet().then((response) => response.data),
    queryKey: [queryKeys.SAVED_RECIPE_LIST],
  });

  const data: Recipe[] | undefined =
    recipeList.data && savedRecipes.data
